...
\hangbibentry{stachelekGranularMeasuresAgricultural2020}
...
Both TN and TP concentrations were sensitive to measures of watershed land-use as well as near-stream agriculture (Figure \ref{fig2}, Table \ref{table2.2}). Despite these similarities, we found differences in both the magnitude of these effects and the extent to which we observed regional variation in watershed land-use sensitivity for N and P models (Figure \ref{fig2.4}). For P, there was little evidence that sensitivity to watershed land-use was regionally variable. More specifically, the credible intervals for the slope of each individual region overlapped the global slope estimate (Figure \ref{fig2.4}). In contrast, for N we found evidence for regional variation in sensitivity to watershed land-use. For this nutrient, the credible intervals for 2 of the 37 regions did not overlap the global estimate (Figure \ref{fig2.4}). These regions were found in parts of Iowa, Minnesota, and Illinois and appear to be more sensitive to watershed land-use – i.e., lake N increases at a faster rate per unit increase in agricultural land-use compared to other regions (Figure \ref{fig2.5}). The median soil clay content of watersheds in these regions was higher than the median across watersheds in all other regions (Figure \ref{fig2.6}). Furthermore, these two regions had a unique combination of both high soil clay content and extensive tile drainage (Figure S2.\ref{figs2.6}).
...
\begin{figure}
  ...
  \end{center}
  \caption{Location of hydrologic regions sensitive to watershed land-use cover corresponding to highlighted credible intervals in Figure \ref{fig2.4}}
  \label{fig2.5}
\end{figure}
...
\begin{figure}
  ...
  \end{center}
  \caption{Histograms showing the distribution of soil clay content for watersheds in regions sensitive to watershed land-use (see highlighted credible intervals in Figure \ref{fig2.4}) relative to watersheds all other regions. Medians for each group are shown as vertical dashed lines.}\label{fig2.6}
\end{figure}
...
\def\suppfigurename{Figure S2}
\def\supptablename{Table S2}

\begin{suppfigure}[h]
  \begin{center}
    \includegraphics[width=0.7\textwidth, height=3.5in,keepaspectratio]{figures/cdl_vs_nlcd-1}
  \end{center}
  \caption{Example of increasing granularity for total Ag to Ag versus pasture, to pasture versus specific crops. For illustration, only corn, soybeans, and pasture are shown rather than all CDL land-use categories.}\label{figs2.1}
  \end{suppfigure}
...
Corn & Corn\\
Corn & Sweet corn\\
Corn & Pop or orn corn\\
Corn & Non irrigated corn\\
Forest & Forest\\
...
Soybeans & Soybeans\\
Soybeans & Non irrigated soybeans\\
Wetlands & Wetlands\\
Wetlands & Woody wetlands\\
Wetlands & Herbaceous wetlands\\
\bottomrule
\end{tabular}
  \caption{Category definitions from the 2010 CDL. See code supplement for listing of variables classified as 'ag' (\href{https://doi.org/10.5281/zenodo.3754916}{doi:10.5281/zenodo.3754916}).} \label{tables2.1}
\end{supptable}

\begin{suppfigure}[h]
  \begin{center}
    \includegraphics[width=0.7\textwidth, height=3.5in,keepaspectratio]{figures/ag-scatter-1}
  \end{center}
  \caption{Lake nutrient concentrations plotted against percent watershed agriculture.}\label{figs2.2}
\end{suppfigure}

\begin{suppfigure}[h]
  \begin{center}
    \includegraphics[width=0.5\textwidth, height=3.5in,keepaspectratio]{figures/trophic_state-1}
  \end{center}
  \caption{Lake trophic state in our study lakes versus all lakes from \citet{sorannoLAGOSNEMultiscaledGeospatial2017} located within our study extent. Trophic state based on the chlorophyll criteria from \citet{carlson1996coordinator}.}\label{figs2.3}
\end{suppfigure}

\begin{suppfigure}[h]
  \begin{center}
    \includegraphics[width=0.9\textwidth, height=3.5in,keepaspectratio]{figures/fe_nolulc-1}
  \end{center}
  \caption{Global (fixed effect) coefficient values and credible intervals for top-ranked lake TP and TN models when land-use predictors are excluded. Values shown are posterior medians (filled circles) and 95\% credible intervals (solid lines). Also shown is a comparison to a zero effect (solid vertical line). Values that do not overlap zero are shaded in red. Horizontal bars separate coefficients in distinct predictor categories. Coefficient estimates are reported relative to standardized predictor variables centered at zero with unit variance and correspond with $\beta$ from Equation \ref{eqn2.1}.}\label{figs2.4}
\end{suppfigure}

\begin{suppfigure}[h]
  \begin{center}
    \includegraphics[width=0.7\textwidth, height=3.5in,keepaspectratio]{figures/08_exploratory_dotplot-1}
  \end{center}
  \caption{Heatmap showing Pearson correlation coefficients among predictor variables. Grey cells denote correlation matrix diagonals.}\label{figs2.5}
\end{suppfigure}

\begin{suppfigure}[h]
  \begin{center}
    \includegraphics[width=0.55\textwidth, height=3.5in,keepaspectratio]{figures/tile_drainage_vs_clay-1}
  \end{center}
  \caption{Scatterplot showing the median clay content of watersheds in our hydrologic regions plotted against percent tile drainage from \citet{nakagaki2016estimates}. The regions that are highly sensitive to agricultural land-use from Figure 4 are highlighted in red.}\label{figs2.6}
\end{suppfigure}

\begin{suppfigure}[h]
  \begin{center}
    \includegraphics[width=0.6\textwidth, height=3.5in,keepaspectratio]{figures/clay-nonlinear-1}
  \end{center}
  \caption{Scatterplot showing the non-linear relationship between watershed clay content and lake TN concentration. Vertical dashed line shows transition between a positive and a negative correlation (r) between the two variables. Solid red line shows the fit of a generalized additive model from the \texttt{mgcv} R package.}\label{figs2.7}
\end{suppfigure}
